...
import {
    ...
    coursePageStyles} from '../assets/dummyStyles'
import courses from '../assets/dummyData'
import {useNavigate} from 'react-router-dom'
import {Star,StarHalf,Search,User,X,StarIcon,SmilePlus} from 'lucide-react'
import {toast,Slide, ToastContainer} from 'react-toastify'
    ...
    const StartIcon=({filled =false,half =false,className=""})=>{
        if(half){
            return <StarHalf className={`w-4 h-4 ${className}`} fill='currentColor'></StarHalf>
        }
        return(
            <Star className={`w-4 h-4 ${className}`} fill={filled ? "currentColor":"none"}></Star>
        )
    };
